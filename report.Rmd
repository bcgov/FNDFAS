...
### Nitrogen (N):
`r concat_comments_nutrients("N", N_norm(), input$Prev_fert, input$Lab_type)`


### Phosphorus (P):
`r concat_comments_nutrients("P", P_norm(), input$Prev_fert, input$Lab_type)`

`r concat_comments_nutrients_ratio("N:P", N_norm()/P_norm(), P_norm(), SO4_norm(), input$Prev_fert)`

### Potassium (K):
`r concat_comments_nutrients("K", K_norm(), input$Prev_fert, input$Lab_type)`

`r concat_comments_nutrients_ratio("N:K", N_norm()/K_norm(), K_norm(), SO4_norm(), input$Prev_fert)`

### Calcium (Ca):
`r concat_comments_nutrients("Ca", Ca_norm(), input$Prev_fert, input$Lab_type)`


### Magnesium (Mg):
`r concat_comments_nutrients("Mg", Mg_norm(), input$Prev_fert, input$Lab_type)`
`r concat_comments_nutrients_ratio("N:Mg", N_norm()/Mg_norm(), Mg_norm(), SO4_norm(), input$Prev_fert)`

### Sulphur (S):
`r concat_comments_nutrients("S", S_norm(), input$Prev_fert, input$Lab_type)`
`r concat_comments_nutrients_ratio("N:S", N_norm()/S_norm(), S_norm(), SO4_norm(), input$Prev_fert)`
`r output$diag_SO4 <- renderText(concat_comments_nutrients("SO4", SO4_norm(), input$Prev_fert, input$Lab_type))`

### Copper (Cu):
`r concat_comments_nutrients("Cu", Cu_norm(), input$Prev_fert, input$Lab_type)`


### Zinc (Zn):
`r concat_comments_nutrients("Zn", Zn_norm(), input$Prev_fert, input$Lab_type)`

### Iron (Fe):
`r concat_comments_nutrients("Fe", Fe_norm(), input$Prev_fert, input$Lab_type)`

### Manganese (Mn):
`r concat_comments_nutrients("Mn", Mn_norm(), input$Prev_fert, input$Lab_type)`

### Boron (B):
`r concat_comments_nutrients("B", B_norm(), input$Prev_fert, input$Lab_type)`
